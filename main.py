...
async def on_message(message):
    ...
    if message.content.startswith("!cursed"):
        await message.channel.send(
            "the box, you opened it, you fool, you opened the box"
            "and now you must pay the price for your foolishness"
    ...
    if message.content.startswith("!help"):
        embed = discord.Embed(
            ...
            description="!random map : get random map\n"
            ...
            "!survivor one : get 1 random survivor perk\n"
            "!killer one : get 1 random killer perk",
            color=0x00FF00,
        )
    ...
    if message.content.startswith("!survivor one"):
        perk = survivorone[0]
        embed = discord.Embed(
            title=perk["perk_name"], description=perk["name"], color=0x00FF00
        )
    ...
    if message.content.startswith("!killer one"):
        perk = killerone[0]
        embed = discord.Embed(
            title=perk["perk_name"], description=perk["name"], color=0xFF0000
        )
        embed.set_image(url=perk["icon_url"])
        await message.channel.send(embed=embed)
